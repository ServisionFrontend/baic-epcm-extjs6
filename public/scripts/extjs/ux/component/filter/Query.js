Ext.define('Ext.ux.component.filter.Query', {
    ...
    autoMetricsLabelWidth: function() {
        ...
        Ext.each(fields, function(item) {
            var labelWidth = tm.getWidth(item.fieldLabel + ':') + 5 + (item.allowBlank ? 0 : 10);

            boxWidth = boxWidth + (item.allowBlank ? 0 : 10);
    ...
    getFilters: function() {
        ...
        Ext.each(fields, function(item) {
            ...
            if (!me.isEmptyValue(value)) {
                params[item.name] = item.toUppercase ? Ext.util.Format.uppercase(value) : value;
            }
        });
