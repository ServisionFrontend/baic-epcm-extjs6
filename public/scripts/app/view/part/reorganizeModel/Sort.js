Ext.define('APP.view.part.reorganizeModel.Sort', {
  ...
  getGridData: function (code) {
    ...
    Ext.util.ajax({
      url: APP.globalConfig.restpath + '/selector/list/model?modelGroupCode=' + code + '&_dc=' + Math.random(),
      method: 'GET',
      noCache: true,
  ...
  saveSort: function () {
    ...
    if (data) {
      Ext.util.ajax({
        url: APP.globalConfig.restpath + '/sort/index/model',
        method: 'PUT',
        jsonData: data,
        beforerequest: function () {
          me.setLoading('加载中,请稍候...');
        ...
        success: function (root) {
          var selector = me.down('[itemId=selector]');

          me.getGridData(selector.getValue());
  ...
  items: [{
    ...
    items: [{
      ...
      items: [{
        ...
        url: APP.globalConfig.restpath + '/selector/list/model-group?seriesCode={id}&sort=name',
        value: '',
        allowBlank: false
      }]
    }, {
