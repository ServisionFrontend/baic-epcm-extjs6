Ext.define('APP.view.dictionary.notice.Sort', {
  ...
  getGridData: function (code) {
    ...
    Ext.util.ajax({
      url: APP.globalConfig.restpath + '/selector/multiPath?path=admin/message/type' + '&_dc=' + Math.random(),
      method: 'GET',
  ...
  saveSort: function () {
    ...
    if (data) {
      Ext.util.ajax({
        url: APP.globalConfig.restpath + '/sort/multiPath?path=admin/message/type',
        method: 'PUT',
        jsonData: data,
        beforerequest: function () {
          me.setLoading('加载中,请稍候...');
